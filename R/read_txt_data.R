...
read.txt.data = function(path = ".",
  ...
  #   mutate(nums = stringr::str_extract(nm, "[:digit:]+"),
  ...
  #                       NA_character_),
  #          date = lubridate::ymd(paste0("20", yr, "0101")) + lubridate::days(jday - 1)) %>%
  ...
  #     #          tag_id = if_else(stringr::str_sub(tmp, -1) %in% c(0, 1, 2),
  #     #                           as.integer(paste0(stringr::str_sub(tmp, 1, -2), 0)),
  #     #                           if_else(stringr::str_sub(tmp, -1) %in% c(8, 9),
  #     #                                   as.integer(ceiling(as.numeric(tmp) / 10) * 10),
  #     #                                   as.integer(paste0(stringr::str_sub(tmp, 1, -2), 5))))) %>%
