...
```{r diagnostics, eval = F, echo = F}
library(mcmcr)
my_mcmcr = as.mcmcr(cjs_post)
...
```{r alternative-method, echo = F, eval = F}
library(marked)
library(ggrepel)
...
ch_ddl = make.design.data(ch_proc)

mod = crm(data = ch_proc,
          ddl = ch_ddl,
          model.parameters = list(Phi = list(formula = ~ -1 + time),
                                  p = list(formula = ~ -1 + time)))

marked_res = mod$results$reals %>%
  map_df(.id = 'param',
         .f = as_tibble) %>%
  mutate(site_num = if_else(param == 'p',
                            occ,
                            as.integer(occ + 1))) %>%
  left_join(tibble(site = colnames(jags_data$y)) %>%
              mutate(site = factor(site, levels = site),
...
marked_res %<>% 
  bind_rows(marked_res %>%
              filter(param == 'Phi') %>%
              arrange(site_num) %>%
              mutate(param = 'survship') %>%
              mutate(estimate = cumprod(estimate)))
...
param_summ %>%
  mutate(param_grp = recode(param_grp,
                            'phi' = 'Phi')) %>%
  select(-param) %>%
  left_join(marked_res %>%
              select(param_grp = param,
                     site,
                     estimate)) %>%
  filter(!is.na(estimate)) %>%
  filter(site_num < max(site_num)) %>%
  # filter(estimate < 0.99) %>%
