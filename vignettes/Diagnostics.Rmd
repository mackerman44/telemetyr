...
compressed = compress_raw_data(raw_df,
                                min_yr = 2018,
                                max_yr = 2019,
                                round_to = 5)
...
```{r volt-temp, eval = FALSE}
# read receiver volt & temperature info
volt_temp_df = read_volt_temp_data(download_path)

```

```{r load-volt-temp-data, echo = F}
load("../data/prepped/2018_2019/volt_temp.rda")
...
temp_p = plot_volt_temp_data(volt_temp_df,
                             column = "temp_avg",
                             receiver_codes = receiver_nms)
